    ...
    stage('Config target') {
      ...
      steps {
        ...
        sh (returnStdout: false, script: '''
          ...
          kubectl exec --namespace kube-system mysql-0 -- mysql -h 127.0.0.1 -uroot -p$PASSWORD -P3306 -e "create database if not exists archivement_manager;"
          ...
          for vhost in `cat cmd/*/*.viper.yaml | grep hostname | awk '{print $2}' | sed 's/"//g' | sed 's/\\./-/g'`; do
            ...
            ./apollo-item-config.sh $APP_ID $TARGET_ENV $vhost database_name archivement_manager
            cd -
          done
        '''.stripIndent())
      }
